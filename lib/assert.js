...
function assertResult(core) {
  try {
    ...
    // stackql emits the literal `null` for a query that matches no rows;
    // normalise that (and any non-array result) to an empty array so an
    // empty result set is a clean assertion failure, not a crash on .length.
    const parsed = parseResult(RESULT);
    const actualResult = Array.isArray(parsed) ? parsed : [];
